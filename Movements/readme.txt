Writing a program to control turtle's movements
ROS Distro: Noetic on Ubuntu 20.04

'start.sh' starts ROSMASTER, TurtleSim and displays meassages from 'turtle1/pose' topic in a new terminal for tracking purposes

move_forward(distance) should calculate the position (x or y) based on a distance given + initial position
For now there is only in x direction... 
